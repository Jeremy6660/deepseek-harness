...
function mount(
  ...
  const renderSlot = ((key: string, owner: object, opts?: { only?: string; fallback?: ReactNode }) => {
    ...
    if (key === 'conversation.hero.welcome') { return opts?.fallback ?? null }
...
describe('Hero chrome', () => {
  it('renders the English preview badge through the hero locale seat', () => {
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>((key: string, _owner: unknown, opts?: { fallback?: ReactNode }) => (
      key === 'conversation.hero.welcome' ? (opts?.fallback ?? null) : null
    ))
