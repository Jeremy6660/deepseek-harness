...
A package joins the table by declaring `dsh.client` (`platform: 'web'`, optional `inject` edges, optional `immediately`) in its package.json and exporting its built bundle at `exports["./client"]`. Each live row resolves from its own Loader specifier and owning-tree `baseUrl`, through the same `loader.internal.resolveSync` implementation that imports its Host face when available. The nearest owning package manifest supplies the browser module id, so relative source and built overlays retain the package identity. A packaged executable cannot point an operating-system link into its own virtual filesystem and materializes each package it carries as a redirect-only module proxy instead, so a manifest whose `dsh.moduleFallback.targets` names a root is followed to the package it stands in for: the row takes that package's `dsh.client` declaration and its real `./client` bundle, never the proxy's re-export shim. Following is bounded, so a proxy cycle leaves the package unclassified rather than looping. Distinct active Loader sources resolving to one package name fail composition; after one source unloads, the surviving source supplies the row without a fiber restart.
