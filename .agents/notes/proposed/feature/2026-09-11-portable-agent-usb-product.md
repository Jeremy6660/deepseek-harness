...
The target is confirmed, not proposed: the publisher sells the product for real money at low margin and accepts the support obligation, with no deadline and no sale of a partial product. Per-user installation and uninstallation, media preparation, integrity verification, repair, backup and recovery, the credential vault, the offline toolkit, and release qualification are all in scope and must be implemented; the exclusions stated below are unchanged.

## Proposal

Create an unofficial, independently named portable agent USB product based on DeepSeek Harness. The shipped identity is **X-FUSION** (product id `x-fusion`), fixed through one product configuration; **Portable Agent Lab USB** is the retired working name that this note's file and symbol names still carry. Product copy must state that DeepSeek does not publish or endorse the product and must identify the exact upstream source revision.
