...
便携命令使用已忽略的 `.dsh-build/` 与 `dist-exe/` 下的目录。部署前，它会同时对照产品环境与生成产物的摘要校验客户端构建记录，使用 `--skip-build` 复用完整品牌化构建时也会校验。记录缺失或不匹配会在打包前失败，防止官方品牌或其他产品的品牌被记为当前产品构建。Runtime 文件是发行版封装的输入，封装会以 `runtime` 角色记录它们；构建命令不封装发行版，也不写入介质。
