...
The portable command adds two repository roots under `.dsh-build/` and `dist-exe/`, both already ignored, and it must run the complete client build before it packages; `--skip-build` exists for the case where those artifacts already belong to the staging root's metadata and is not a supported way to package a different product's artifacts. The Runtime files are inputs to distribution sealing, which records them under the `runtime` role; nothing in this change seals a distribution or writes media.
