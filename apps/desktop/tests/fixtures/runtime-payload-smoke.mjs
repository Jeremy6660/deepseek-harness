...
/** Confirm the runtime carries the stable system-primitives entry package. */
function checkSystemAddonContract() {
  const manifest = requireRuntime('@deepseek-ai/node-addon-system/package.json')
  assert.equal(manifest.name, '@deepseek-ai/node-addon-system')
  assert.ok(manifest.exports?.['./flock'], 'system-primitives package exports its flock contract')
}
...
process.once('beforeExit', () => {
  console.log(JSON.stringify({ node: process.versions.node, platform: process.platform, arch: process.arch,
    systemAddon: true, koffi: true, sharp: true, html: true, pty: true }))
})
