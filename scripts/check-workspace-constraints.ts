...
const standardReleaseMemberDirectory =
  /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/(?!(?:desktop(?:-host)?|portable)$)[^/]+|vendor\/[^/]+)$/
/** Product applications assembled into platform artifacts rather than published to npm. */
const privateApplicationDirectories = new Set(['apps/desktop', 'apps/portable'])
